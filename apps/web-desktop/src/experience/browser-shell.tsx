      ...
      <aside id="browser-navigation" ref={drawer} className={`browser-navigation ${drawerOpen ? 'is-open' : ''}`} aria-label="Sessions, Bots and tools">
        ...
        <div className="browser-navigation-body" role="tabpanel" aria-label={tab}>
          ...
          {tab === 'tools' && <nav className="browser-tools" aria-label="Tools">
            <p>Tools</p><button className="browser-tool-row" aria-label="settings" aria-current={location.pathname === '/settings' ? 'page' : undefined} onClick={() => openRoute('/settings')}><span className="browser-tool-icon"><Codicon name="settings-gear" size="1.25rem" /></span><span>Settings</span></button>{APP_ROUTES.filter(route => TOOLS_ROUTE_IDS.has(route.id)).map(route => <button className="browser-tool-row" key={route.path} aria-label={route.id} aria-current={location.pathname === route.path ? 'page' : undefined} onClick={() => openRoute(route.path)}><span className="browser-tool-icon"><Codicon name={toolRouteIcon(route.id)} size="1.25rem" /></span><span>{toolRouteLabel(route.id)}</span></button>)}
            <p>Workspace</p>{APP_ROUTES.filter(route => !['new', 'settings', 'session-import', ...TOOLS_ROUTE_IDS].includes(route.id)).map(route => <button className="browser-tool-row" key={route.path} aria-label={route.id.replaceAll('-', ' ')} aria-current={location.pathname === route.path ? 'page' : undefined} onClick={() => openRoute(route.path)}><span className="browser-tool-icon"><Codicon name={toolRouteIcon(route.id)} size="1.25rem" /></span><span>{sentenceCase(route.id)}</span></button>)}
