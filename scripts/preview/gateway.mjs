...
export function createPreviewGateway({ log = () => {}, delay = 95 } = {}) {
  ...
  const page = (profile = 'all', all = false) => {
    const rows = [...sessions.values()].filter(row => (all || !row.hidden) && (profile === 'all' || row.profile === profile))
  ...
  function rpc(method, params = {}) {
    ...
    if (method === 'session.resume') return snapshot(id)
    ...
    if (method === 'session.rename' && sessions.has(id)) sessions.get(id).title = params.title
  ...
  sockets.on('connection', client => {
    ...
    client.on('message', data => {
      try {
        const message = JSON.parse(String(data)); const result = rpc(message.method, message.params)
        if (message.id !== undefined) client.send(JSON.stringify({ jsonrpc: '2.0', id: message.id, result }))
      } catch (error) { log('ERROR', error.message) }
    })
  })
  return { server, sessions, messages, close: async () => { for (const timer of timers.values()) clearInterval(timer); for (const client of clients) client.terminate(); sockets.close(); server.closeAllConnections(); await new Promise(resolve => server.close(resolve)) } }
}
