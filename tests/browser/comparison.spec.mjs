...
test('browser drawer, repeated Bot selection, Tools and profile survive comparison switching', async ({ page }) => {
  ...
  await page.getByRole('tab', { name: 'Tools', exact: true }).click()
  const toolsNavigation = page.getByRole('navigation', { name: 'Tools', exact: true })
  await expect(toolsNavigation).toBeVisible()
  await expect(toolsNavigation.getByText('System', { exact: true })).toBeVisible()
  ...
  await expect(profile).toHaveValue('research')
  await expect(profile.locator('option').first()).toHaveText('All')
  await expect(profile.locator('option[value="research"]')).toHaveText('Research')
  await expect(profile.locator('option[value="__new_profile__"]')).toHaveCount(0)
  await page.getByRole('button', { name: 'Profile actions', exact: true }).click()
  const profileMenu = page.getByRole('menu', { name: 'Profile actions', exact: true })
  await expect(profileMenu).toBeVisible()
  await expect(profileMenu.getByRole('menuitem', { name: 'New profile', exact: true })).toBeVisible()
  await expect(profileMenu.getByRole('menuitem', { name: 'Import profile', exact: true })).toBeVisible()
  await expect(profileMenu.getByRole('menuitem', { name: 'Manage profiles', exact: true })).toBeVisible()
  await page.keyboard.press('Escape')
  await expect(profileMenu).toBeHidden()
  await profile.selectOption('__all__')
  await expect(profile).toHaveValue('__all__')
  await expect(page.getByText('Plan a calmer working week', { exact: true })).toBeVisible()
  await expect(page.getByText('Bot Chat', { exact: true }).first()).toBeVisible()
  expect(errors).toEqual([])
})
